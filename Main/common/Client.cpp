...
bool Client::sendPacket(const PACKET_DATA& packet, const DATA_TYPE& type)
{
	size_t data_size = sizeof(PACKET_DATA);
	size_t packet_size = 1 + sizeof(size_t) + sizeof(PACKET_DATA);
	char* buf = new char[packet_size];
	memset(buf, 0, packet_size);
	buf[0] = type;
	memcpy(&buf[1], &data_size, sizeof(size_t));
	memcpy(&buf[1 + sizeof(size_t)], &packet, sizeof(PACKET_DATA));
...
bool Client::sendText(const std::string& msg, const DATA_TYPE& type)
{
	// utf-8 -> char
	string str = msg;// Client::UnicodeToMultibyte(CP_UTF8, send_msg);
